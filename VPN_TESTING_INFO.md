...
**Testing Flow:**
```
1. Extract IP from path (/ip-port) → TCP test → Success ✅
     ↓ (if not found)
2. Extract Host → Clean domain from server → Resolve IP → TCP test → Success ✅  
     ↓ (if fail)
3. Extract SNI → Clean domain from server → Resolve IP → TCP test → Success ✅
     ↓ (if fail)  
4. Extract server → Resolve IP → TCP test → Success ✅
     ↓ (if fail)
5. ICMP ping test → Success ✅
     ↓ (if fail)
6. Mark as Dead ❌
```

**🔧 Domain Cleaning Examples:**
- server="tod.com", host="tod.com.do-v3.bhm69.site" → test "do-v3.bhm69.site"
- server="example.com", sni="sg.example.com" → test "sg"  
- server="test.com", host="test.com" → test "test.com" (sama persis, tetap test)
- server="abc.com", host="different.net" → test "different.net" (berbeda total)
